...
!(function (e, t) {
  ...
  const i = {
    ...
    S = /[^\s\p{P}\p{S}]/u,
    T = r(/^((?![*_])punctSpace)/, "u")
      .replace(/punctSpace/g, R)
      .getRegex(),
    z = /(?!~)[\p{P}\p{S}]/u,
    A = /^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,
    _ = r(A, "u").replace(/punct/g, $).getRegex(),
    P = r(A, "u").replace(/punct/g, z).getRegex(),
    I =
      "^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",
    L = r(I, "gu")
      .replace(/notPunctSpace/g, S)
      .replace(/punctSpace/g, R)
      .replace(/punct/g, $)
      .getRegex(),
    B = r(I, "gu")
    ...
    q = r(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/)
      ...
      .getRegex(),
    Z = r(k).replace("(?:--\x3e|$)", "--\x3e").getRegex(),
    v = r(
    ...
    H = {
      ...G,
      br: r(y).replace("{2,}", "*").getRegex(),
      text: r(G.text)
        .replace("\\b_", "\\b_| {2,}\\n")
        .replace(/\{2,\}/g, "*")
        .getRegex(),
    },
    X = { normal: x, gfm: w, pedantic: m },
    F = { normal: j, gfm: G, breaks: H, pedantic: N },
    U = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" },
    J = (e) => U[e];
  function K(e, t) {
    if (t) {
      if (i.escapeTest.test(e)) return e.replace(i.escapeReplace, J);
    } else if (i.escapeTestNoEncode.test(e)) return e.replace(i.escapeReplaceNoEncode, J);
    return e;
  }
  function V(e) {
    try {
  ...
  class te {
    ...
    reflink(e, t) {
      ...
      if ((n = this.rules.inline.reflink.exec(e)) || (n = this.rules.inline.nolink.exec(e))) {
        ...
        return ee(n, e, n[0], this.lexer, this.rules);
      }
    }
    emStrong(e, t, n = "") {
    ...
    codespan(e) {
      ...
      if (t) {
        ...
        return (
          n && s && (e = e.substring(1, e.length - 1)), { type: "codespan", raw: t[0], text: e }
        );
      }
    }
    br(e) {
  ...
  class ne {
    ...
    constructor(t) {
      ...
      this.options.pedantic
        ? ((n.block = X.pedantic), (n.inline = F.pedantic))
        : this.options.gfm &&
          ((n.block = X.gfm), this.options.breaks ? (n.inline = F.breaks) : (n.inline = F.gfm)),
        (this.tokenizer.rules = n);
    }
    static get rules() {
      return { block: X, inline: F };
    }
    static lex(e, t) {
  ...
  class se {
    ...
    code({ text: e, lang: t, escaped: n }) {
      ...
      return s
        ? '<pre><code class="language-' + K(s) + '">' + (n ? r : K(r, !0)) + "</code></pre>\n"
          ...
          ? "paragraph" === e.tokens[0]?.type
            ? ((e.tokens[0].text = n + " " + e.tokens[0].text),
              e.tokens[0].tokens &&
                ...
                ((e.tokens[0].tokens[0].text = n + " " + K(e.tokens[0].tokens[0].text)),
  ...
  class oe {
    ...
    use(...e) {
      ...
      return (
        e.forEach((e) => {
          ...
          if (e.tokenizer) {
            const t = this.defaults.tokenizer || new te(this.defaults);
    ...
    parseMarkdown(e) {
      return (t, n) => {
        ...
        const l = r.hooks ? r.hooks.provideLexer() : e ? ne.lex : ne.lexInline,
          o = r.hooks ? r.hooks.provideParser() : e ? ie.parse : ie.parseInline;
        if (r.async)
    ...
    onError(e, t) {
      return (n) => {
        if (((n.message += "\nPlease report this to https://github.com/markedjs/marked."), e)) {
          const e = "<p>An error occurred:</p><pre>" + K(n.message + "", !0) + "</pre>";
